...
#data
dat <- read.csv() #349987 records total from NM (includes non-georeferenced)
#load(file="datCleanAllRecords.RData")
...
dat_geo <- dat %>% filter(!decimalLatitude=="", !decimalLongitude=="") #removes 129453 records without coordinates


## remove impossible coordinates
dat_geo <- dat_geo %>% filter(!decimalLatitude>90, !decimalLatitude< -90) #removes 2 impossible records.  
```
...
ggplot()+
  geom_map(data=world.inp, 
           ...
           fill = "grey80")+
  xlim(min(dat_geo$decimalLongitude, na.rm = T), 
       max(dat_geo$decimalLongitude, na.rm = T))+
  ylim(min(dat_geo$decimalLatitude, na.rm = T), 
       max(dat_geo$decimalLatitude, na.rm = T))+
