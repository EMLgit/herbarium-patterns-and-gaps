...
#data
dat <- read.csv() #376030 records total from NM (includes non-georeferenced)
load(file=)
...
dat_geo <- dat %>% filter(!decimalLatitude=="", !decimalLongitude=="") #removes 129,147 records


## remove impossible coordinates
dat_geo <- dat_geo %>% filter(!decimalLatitude>90, !decimalLatitude< -90) #removes impossible records. Zero removed. 

```

#Check the spatial alignment
```{r mapping}
world.inp  <- map_data("world")

#check coordinate alignment on a map
ggplot()+
  geom_map(data=world.inp, 
           map=world.inp, 
           aes(x=long, y=lat, map_id=region), 
           fill = "grey80")+
  xlim(min(dat.geo$decimalLongitude, na.rm = T), 
       max(dat.geo$decimalLongitude, na.rm = T))+
  ylim(min(dat.geo$decimalLatitude, na.rm = T), 
       max(dat.geo$decimalLatitude, na.rm = T))+
  geom_point(data = dat.geo, 
             aes(x = decimalLongitude, y = decimalLatitude, color=sourceDB),
             size = 1)+
  coord_fixed()+
  theme_bw()+
...
#clip to NM boundary
dat_sf <- dat_sf[nm.geometry, ]
plot(dat_sf$geometry) #fixed! Removed 1343 records



#extract coords from sf
dat.coords<-st_coordinates(dat_sf)
dat_sf<-cbind(dat_sf, dat.coords)

#rename latitude and longitude in seinet
dat_sf <- dat_sf %>%
  rename(decimalLongitude=X.1) %>%
  st_drop_geometry()
...
dat_sf <- dat_sf %>% filter(coordinateUncertaintyInMeters/1000 <= 100 | is.na(coordinateUncertaintyInMeters)) #Removed 236 records 


## remove unsuitable data sources, especially fossils
table(dat_sf$basisOfRecord) #Why are there any human observations involved???? It just happens. Messes and stuff.
dat_sf <- dat_sf %>% filter(!basisOfRecord=="HumanObservation", !basisOfRecord=="Observation", !basisOfRecord=="") #this removes 1834 records 
...
save(dat_sf, file="GitHub_script3_georefCleaned.RData")
