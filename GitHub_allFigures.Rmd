...
##Days of the week variation
weekday.dat <- as.data.frame(weekdays(dat$date))
table(weekday.dat)
...
physio2<- env.v %>%
  ...
  as.data.frame() 
#%>% dplyr::select(-geometry) 
physio2$cnts <- aggregate(physio$cnt, list(physio$PROVINCE), FUN=sum)
physio2$area.kms <- set_units(physio2$area, "km^2") #Add a column with area in kilometers squared


physio.prop <- ggplot(physio2, aes(x=area.kms, y=cnts$x, color=physio)) +
